...
class ProteinLSTMEncoder(nn.Module):
    ...
    def reverse_sequence(self, sequence, input_mask):
        if input_mask is None:
            idx = torch.arange(sequence.size(1) - 1, -1, -1).cuda()
            ...
            for seq, seqlen in zip(sequence, sequence_lengths):
                idx = torch.arange(seqlen - 1, -1, -1).cuda()
...
class ProteinLSTMModelProsit(nn.Module):
    ...
    def forward(self, sequence, input_mask=None):
        if input_mask is None:
            input_mask = torch.ones_like(sequence)

        # fp16 compatibility
        #embedding_output = self.embed_matrix(input_ids)
        outputs = self.encoder(sequence, input_mask=None)
